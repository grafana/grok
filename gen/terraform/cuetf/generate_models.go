...
// func generateAttributes(nodes []types.Node) (string, string, error) {
// 	attributesContent := ""
// 	structContent := ""
...
// 			if subType != "" {
// 				attributesContent += fmt.Sprintf("	%s := []%s{}\n", identifier, subTypeGolang)
// 				attributesContent += fmt.Sprintf("	for _, v := range d.%s.Elements() {\n", ToCamelCase(node.Name))
// 				attributesContent += fmt.Sprintf("		%s = append(%s, v.(types.%s).%s)\n", identifier, identifier, subType, subTypeFunc)
// 				attributesContent += "	}\n"
// 			} else {
// 				attributesContent += fmt.Sprintf("	%s := []struct{\n", identifier)
// 				jsonModelFields, err := GenerateJSONModelFields(node.Children)
// 				if err != nil {
// 					return "", "", err
// 				}
// 				attributesContent += jsonModelFields
// 				attributesContent += "\n	}{}\n"

// 				// _, _, err := generateAttributes(node.Children)
// 				if err != nil {
// 					return "", "", err
// 				}
...
// 			attributesContent += jsonModelFields
// 			attributesContent += "\n	}{\n"
// 			_, nestedStructContent, err := generateAttributes(node.Children)
// 			if err != nil {
// 				return "", "", err
// 			}
// 			attributesContent += nestedStructContent
// 			attributesContent += "	}\n"
// 			// attributesContent += nestedAttributesContent
// 		} else if funcString != "" {
// 			attributesContent += fmt.Sprintf("	%[1]s := d.%[2]s.%[3]s\n", identifier, ToCamelCase(node.Name), funcString)
// 			if node.Optional {
// 				ref = "&"
// 			}
// 		} else {
...
// 		structContent += fmt.Sprintf("		%[1]s: %s%s,\n", ToCamelCase(node.Name), ref, identifier)
// 	}

// 	return attributesContent, structContent, nil
// }
